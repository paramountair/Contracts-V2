...
function newContractView() {
  ...
  <h2>Asset list</h2><div id="assetEditor"></div><div class="action-row"><button class="secondary" onclick="addAssetRow()">Add asset row</button><button class="secondary" onclick="generateContract(false)">Preview Proposal</button><button class="primary" onclick="generateContract(true)">Generate & Download Proposal</button></div><p class="muted">Generate & Download creates a client-facing proposal file automatically, then opens the proposal preview for review.</p></div>`;
}
...
function proposalView(){ const c=selectedContract(); return `<div class="page-head no-print"><div><span class="pill navy">CUSTOMER OUTPUT</span><h1>Proposal Preview</h1><p>This is the client-facing proposal created from the contract setup. Download the proposal file or print/save as PDF.</p></div><div><button class="secondary" onclick="downloadProposal()">Download HTML</button><button class="primary" onclick="printProposal()">Print / Save as PDF</button></div></div>${proposalDocument(c)}`; }
